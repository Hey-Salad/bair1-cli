...
function aqiLabel(aqi: number): string {
  if (aqi <= 50) return `${GREEN}Good${RESET}`;
  if (aqi <= 100) return `${YELLOW}Moderate${RESET}`;
  if (aqi <= 150) return `${YELLOW}Unhealthy for Sensitive Groups${RESET}`;
  if (aqi <= 200) return `${RED}Unhealthy${RESET}`;
  if (aqi <= 300) return `${RED}Very Unhealthy${RESET}`;
...
function printReading(r: Record<string, unknown>): void {
...
function printTable(rows: Record<string, unknown>[], columns: { key: string; label: string; width: number }[]): void {
  // Header
...
program
  ...
  .action(async (opts: { device?: string }) => {
    try {
      ...
      const reading = (Array.isArray(data) ? data[0] : data) as Record<string, unknown>;
...
program
  ...
  .action(async (opts: { device: string; limit: string }) => {
    try {
...
program
  ...
  .action(async (opts: { device: string; from?: string; to?: string; format: string }) => {
    try {
      const params: Record<string, string> = {
        device: opts.device,
        format: opts.format,
...
program
  ...
  .action(async () => {
    try {
      ...
      const reading = (Array.isArray(data) ? data[0] : data) as Record<string, unknown> | undefined;
      ...
      console.log(`  ${BOLD}Error:${RESET}     ${(err as Error).message}`);
      console.log("");
      process.exit(1);
    }
  });
